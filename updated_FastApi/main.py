...
@app.post("/convert/doc/to/pdf/")
async def convert_doc_to_pdf(file: UploadFile , background_tasks: BackgroundTasks = BackgroundTasks()):
    try:

        doc_file_name = file.filename
        doc_file_path = os.path.join(docx_to_pdf_original, doc_file_name ) 
        ...
        background_tasks.add_task(delete_doc_and_pdf,  pdf_path)
...
async def delete_doc_and_pdf( pdf_path ):
    await asyncio.sleep(300)  # Wait for 500 seconds (5 minute)
